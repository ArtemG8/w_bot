...
from keyboards.flow_kb import admin_main_menu_keyboard, admin_choose_card_keyboard, admin_stopped_cards_menu_keyboard, main_menu_keyboard
...
async def process_admin_command(message: Message, state: FSMContext):
    await message.answer(LEXICON_RU['admin_password_request'], reply_markup=ReplyKeyboardRemove())
...
async def process_admin_password_entry(message: Message, state: FSMContext):
    ...
    if entered_password == stored_password:
        await message.answer(LEXICON_RU['admin_welcome'], reply_markup=admin_main_menu_keyboard())
        await state.set_state(Admin.admin_main_menu)
    else:
        await message.answer(LEXICON_RU['admin_incorrect_password'])
        await state.clear()

# --- Admin Main Menu ---
@router.message(F.text == LEXICON_RU['admin_button_back_to_main_menu'], Admin.admin_main_menu)
@router.callback_query(F.data == "admin_main_menu", Admin.choosing_card_to_edit) # Для возврата из выбора карты
@router.callback_query(F.data == "admin_main_menu", Admin.manage_stopped_cards_menu) # Для возврата из стопнутых карт
async def show_admin_main_menu(message_or_callback: Message | CallbackQuery, state: FSMContext):
    if isinstance(message_or_callback, CallbackQuery):
        await message_or_callback.message.edit_reply_markup(reply_markup=None)
        await message_or_callback.message.answer(LEXICON_RU['admin_main_menu_text'], reply_markup=admin_main_menu_keyboard())
        await message_or_callback.answer()
    else:
        await message_or_callback.answer(LEXICON_RU['admin_main_menu_text'], reply_markup=admin_main_menu_keyboard())
    await state.set_state(Admin.admin_main_menu)

# --- Go to main bot menu from admin panel ---
@router.message(F.text == LEXICON_RU['admin_button_back_to_main_menu'], Admin.admin_main_menu)
async def admin_back_to_main_menu(message: Message, state: FSMContext):
...
async def request_new_admin_password(message: Message, state: FSMContext):
    await message.answer(LEXICON_RU['admin_new_password_request'], reply_markup=ReplyKeyboardRemove())
...
async def set_new_admin_password(message: Message, state: FSMContext):
    ...
    if new_password:
        ...
        await message.answer(LEXICON_RU['admin_password_changed'], reply_markup=admin_main_menu_keyboard())
        await state.set_state(Admin.admin_main_menu)
    else:
        await message.answer(LEXICON_RU['admin_no_changes'], reply_markup=admin_main_menu_keyboard())
        await state.set_state(Admin.admin_main_menu)
...
async def request_personal_requisites_link(message: Message, state: FSMContext):
    await message.answer(LEXICON_RU['admin_personal_link_request'], reply_markup=ReplyKeyboardRemove())
...
async def set_personal_requisites_link(message: Message, state: FSMContext):
    ...
    if new_link == '-':
        ...
        await message.answer(LEXICON_RU['admin_personal_link_removed'], reply_markup=admin_main_menu_keyboard())
    elif new_link:
        await update_personal_requisites_link(new_link)
        await message.answer(LEXICON_RU['admin_personal_link_saved'], reply_markup=admin_main_menu_keyboard())
    else:
        await message.answer(LEXICON_RU['admin_no_changes'], reply_markup=admin_main_menu_keyboard())
    await state.set_state(Admin.admin_main_menu)
...
async def start_editing_card(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(LEXICON_RU['admin_request_card_number'].format(card_order=card_order))
...
async def process_card_number(message: Message, state: FSMContext):
    ...
    await message.answer(LEXICON_RU['admin_request_card_name'].format(card_order=card_order))
...
async def process_card_name(message: Message, state: FSMContext):
    ...
    await message.answer(LEXICON_RU['admin_request_bank_name'].format(card_order=card_order))
...
async def process_bank_name(message: Message, state: FSMContext):
    ...
    await message.answer(LEXICON_RU['admin_request_min_amount'].format(card_order=card_order))
...
async def process_min_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(LEXICON_RU['admin_request_max_amount'].format(card_order=card_order))
        await state.set_state(Admin.waiting_for_max_amount)
    except ValueError:
        await message.answer(LEXICON_RU['admin_invalid_amount'])
...
async def process_max_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(LEXICON_RU['admin_request_percentage'].format(card_order=card_order))
...
async def process_percentage(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(LEXICON_RU['admin_requisite_updated_success'].format(card_order=card_order), reply_markup=admin_main_menu_keyboard())
        await state.set_state(Admin.admin_main_menu)
        ...
        if old_requisite and (old_requisite['bank_name'] != user_data['temp_bank_name'] or old_requisite['card_number'] != user_data['temp_card_number']):
            try:
                await message.bot.send_message(
                    ...
                    text=LEXICON_RU['admin_card_stopped_notification'].format(
                        card_order=card_order,
                        bank_name=user_data['temp_bank_name']
                    )
                )
            except Exception as e:
                logger.error(f"Failed to send team chat notification: {e}")

    except ValueError:
        await message.answer(LEXICON_RU['admin_invalid_percentage'])
    except Exception as e:
        logger.error(f"Error updating requisite: {e}")
        await message.answer("Произошла ошибка при обновлении реквизитов.", reply_markup=admin_main_menu_keyboard())
        await state.set_state(Admin.admin_main_menu)
...
async def request_stopped_card_to_add(message: Message, state: FSMContext):
    await message.answer(LEXICON_RU['admin_request_stopped_card_to_add'], reply_markup=ReplyKeyboardRemove())
...
async def add_new_stopped_card(message: Message, state: FSMContext):
    ...
    if card_number:
        ...
        await message.answer(LEXICON_RU['admin_stopped_card_added'].format(card_number=card_number), reply_markup=admin_main_menu_keyboard())
    else:
        await message.answer(LEXICON_RU['admin_no_changes'], reply_markup=admin_main_menu_keyboard())
    await state.set_state(Admin.admin_main_menu) # Возвращаемся в главное меню админки
...
async def request_stopped_card_to_remove(message: Message, state: FSMContext):
    await message.answer(LEXICON_RU['admin_request_stopped_card_to_remove'], reply_markup=ReplyKeyboardRemove())
...
async def remove_existing_stopped_card(message: Message, state: FSMContext):
    ...
    if card_number:
        ...
        if any(card['card_number'] == card_number for card in stopped_cards):
            ...
            await message.answer(LEXICON_RU['admin_stopped_card_removed'].format(card_number=card_number), reply_markup=admin_main_menu_keyboard())
        else:
            await message.answer(LEXICON_RU['admin_stopped_card_not_found'].format(card_number=card_number), reply_markup=admin_main_menu_keyboard())
    else:
        await message.answer(LEXICON_RU['admin_no_changes'], reply_markup=admin_main_menu_keyboard())
    await state.set_state(Admin.admin_main_menu) # Возвращаемся в главное меню админки
